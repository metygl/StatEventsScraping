"""
Scraper for ASA Twin Cities (Minnesota) Chapter Events.
Site: https://amstatmn.org/
Hugo static site with clean semantic HTML.
"""
...
class ASATwinCitiesScraper(BaseScraper):
    """Scraper for ASA Twin Cities chapter events from Hugo static site."""

    SOURCE_NAME = "ASA Twin Cities"
    BASE_URL = "https://amstatmn.org/"

    async def _goto(self, url: str):
        """Navigate using domcontentloaded (site never reaches networkidle)."""
        await self.page.goto(url, wait_until="domcontentloaded", timeout=30000)

    async def scrape(self) -> List[Event]:
        """Scrape ASA Twin Cities event listings."""
        await self._goto(self.BASE_URL)
        await self.wait_for_content("main, article, body", timeout=10000)
        ...
        self._parse_events(body_text, self.BASE_URL)

        # Look for event sub-pages
    ...
    async def _find_event_links(self) -> List[str]:
        ...
        for link in links:
            ...
            if any(kw in text_lower for kw in ["event", "meeting", "course", "workshop", "seminar"]):
                if "amstatmn.org" in href:
                    seen.add(href)
                    urls.append(href)
        return urls
    ...
    def _parse_events(self, text: str, source_url: Optional[str] = None):
        ...
        for match in date_matches:
            date_text = re.sub(r"(\d+)(?:st|nd|rd|th)", r"\1", match.group(1).strip())
            ...
            full_context = before + match.group(0) + after
    ...
    def _extract_title(self, before_text: str) -> Optional[str]:
        ...
        for line in reversed(lines):
            ...
            if len(line) < 10:
                continue
            if re.match(r"^(?:Home|About|Events|Contact|Menu|Navigation|Officers|Newsletter|Links)",
                        line, re.IGNORECASE):
                continue
            return line[:300]
        return None
    ...
    def _extract_speakers(self, text: str) -> List[str]:
        ...
        speakers = []
        match = re.search(r"(?:Speaker|Presenter|Instructor)[s]?[:\s]+([^\n]+)", text, re.IGNORECASE)
        if match:
